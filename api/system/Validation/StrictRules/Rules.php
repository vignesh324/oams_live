...
class Rules
{
    ...
    public function is_unique_without_status($str, string $field, array $data): bool
    {
        if (is_object($str) || is_array($str)) {
            return false;
        }
        ...
        if (
            ...
            $_ignoreValue = strtolower($ignoreValue);
            $row = $row->where("LOWER({$ignoreField}) !=", $_ignoreValue);
        }
    ...
    public function is_unique_with_grade_and_garden($str, string $field, array $data): bool
    {
        ...
        // Extract the parameters from the $field string
        $params = explode(',', $field);

        $field = $params[0];
        $gardenField = $params[1];
        $gardenId = $params[2];
        $gradeField = $params[3];
        $gradeId = $params[4]; // Get all grade IDs from the 5th element onwards
        $ignoreField = $params[count($params) - 2] ?? null;
        $ignoreValue = $params[count($params) - 1] ?? null;

        // echo '<pre>';
        // print_r($gradeId);
        // exit;

        // Extract the table and field names
        sscanf($field, '%[^.].%[^.]', $table, $field);

        // Initialize the database connection
        $db = Config::connect($data['DBGroup'] ?? null);

        // Build the query
        $builder = $db->table($table)
            ->join('inward', 'inward.id = inward_items.inward_id')
            ->select('COUNT(*) AS count')
            ->where($field, $str)
            ->where($gardenField, $gardenId)
            ->where('inward_items.status !=', 0)
            ->Where($gradeField, $gradeId)
            ->limit(1);

        $row = $builder->get()->getRow();

        // $lastQuery = $builder->getCompiledSelect();
        // echo "Last Query: " . $lastQuery . "<br>";
        // exit;

        // Handle the ignore field and value if provided
        if (
            $ignoreField !== null && $ignoreField !== ''
            && $ignoreValue !== null && $ignoreValue !== ''
            && !preg_match('/^\{(\w+)\}$/', $ignoreValue)
            ...
            $_ignoreValue = strtolower($ignoreValue);
            $builder = $builder->where("LOWER({$ignoreField}) !=", $_ignoreValue);
        }
        ...
        return true; // No duplicate entries found
    }


    public function is_unique_with_category($str, string $field, array $data): bool
    {
        if (is_object($str) || is_array($str)) {
            return false;
        }

        [$field, $categoryField, $categoryId, $gradeField, $gradeId, $ignoreField, $ignoreValue] = array_pad(
            explode(',', $field),
            7,
            null
        );
     ...
     * @param array|bool|float|int|object|string|null $str expects int|string
     */
    public function less_than_equal_to($str, string $field, array $data): bool
    {
        if (!array_key_exists($field, $data)) {
            // The comparison field does not exist in the data array
            return false;
        }
