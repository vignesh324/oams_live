...
class Cart extends ResourceController
{
    ...
    public function detail($id)
    {
        ...
        $cartItems = $cart_model->select('auction.lot_count AS auction_lot_count,cart.id as cart_id,cart.qty AS cart_qty,cart.auction_id as auction_id,inward_items.*,grade.name AS grade_name,center.name AS center_name,garden.id AS garden_id,garden.name AS garden_name,warehouse.name AS warehouse_name')
    ...
    public function addTocart()
    {
        ...
        // if ($auction_garden_count > 0) {

        //     $auctiongarden_data = $auctiongardenModel->distinct()->select('garden_id')->where('auction_id', $auction_id)->findAll();

        //     $auction_garden_ids = array();
        //     foreach ($auctiongarden_data as $auction_item) {
        //         $auction_garden_ids[] = $auction_item['garden_id'];
        //     }
        //     sort($auction_garden_ids);
        //     $cart_garden_ids = array();
        //     foreach ($cartdata as $auction_item) {
        //         $cart_garden_ids[] = $auction_item['garden_id'];
        //     }
        //     sort($cart_garden_ids);

        //     $is_same = empty(array_diff($auction_garden_ids, $cart_garden_ids)) && empty(array_diff($cart_garden_ids, $auction_garden_ids));
        //     if ($is_same) {
        //     } else {
        //         $auction_stock = $auctionGardenOrderModel->where('auction_id', $auction_id)
        //             ->delete();
        //     }
        // }
    ...
    public function movetoAuction()
    {
        ...
        $start_time_seconds = strtotime($auctiondata['start_time']);
        $session_count = ceil(count($this->request->getVar('inward_item_id')) / $auctiondata['lot_count']);
        list($session_hours, $session_minutes) = explode(':', $auctiondata['session_time']);
        ...
        for ($i = 0; $i < count($this->request->getVar('inward_item_id')); $i++) {
            ...
            // echo $start_time = $auctiondata['start_time'];
            // exit;
            $auctionItemData = [
    ...
    public function auctiongardenlist()
    {
        ...
        //echo '<pre>';print_r($check_count);exit;
