...
class Auction extends ResourceController
{
    ...
    public function closeAuction()
    {
        ...
        // print_r($grouped_auction_items);exit;
        ...
        foreach ($grouped_auction_items as $buyer_id => $buyer_array) {
            ...
            foreach ($buyer_array as $seller_id => $seller_array) {
                $sellerData = $seller_model->where('id', $seller_id)->first();
        ...
        foreach ($grouped_auction_items_auctioneer as $buyer_id => $buyer_auctioneer_array) {
            ...
            foreach ($buyer_auctioneer_array as $item) {
        ...
        foreach ($grouped_auction_items_seller as $seller_id => $seller_auctioneer_array) {
            ...
            foreach ($seller_auctioneer_array as $item) {
    ...
    public function invoiceDetails($id)
    {
        ...
        if ($invoice) {
            ...
            $invoices = $invoiceitems_model->select('invoice_item.bid_price,invoice_item.qty AS pkgs,
            auction_items.id AS auction_id,auction_items.*,inward_items.id AS inward_id,inward_items.*,
            grade.name AS grade_name,grade.type AS grade_type,center.name AS center_name,garden.name AS garden_name,
            warehouse.name AS warehouse_name,
    ...
    public function auctionBuyerInvoice()
    {
        ...
        $invoices = $invoice_model->select('auction_buyer_invoice.*,buyer.name AS buyer_name,
        buyer.contact_person_number')
    ...
    public function auctionbuyerinvoiceDetails($id)
    {
        ...
        $invoice = $invoiceModel->select('auction_buyer_invoice.*,auction.date AS auction_date,
        buyer.fssai_no AS b_fssai,buyer.tea_board_no AS b_tea,buyer.gst_no AS b_gst,invoice.invoice_no AS tax_invoice_no,
        state.name AS b_state_name,city.name AS b_city_name,area.name AS b_area_name,buyer.state_id AS b_state_id,
        buyer.address AS b_address,auction.sale_no,buyer.name AS buyer_name,buyer.contact_person_number,
        (SELECT delivery_time FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS settings_delivery_time,
        (SELECT buyer_charges FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS settings_buyer_charges
        ')
            ->join('auction', 'auction.id = auction_buyer_invoice.auction_id', 'left')
            ->join('buyer', 'buyer.id = auction_buyer_invoice.buyer_id', 'left')
            ->join('invoice', 'invoice.auction_id = auction_buyer_invoice.auction_id', 'left')
        ...
        if ($invoice) {
            ...
            $invoices = $invoiceitems_model->select('auction_buyer_invoice_item.bid_price,auction_buyer_invoice_item.qty AS pkgs,
            auction_items.id AS auction_id,auction_items.*,inward_items.id AS inward_id,inward_items.*,
            grade.name AS grade_name,grade.type AS grade_type,center.name AS center_name,garden.name AS garden_name,
            warehouse.name AS warehouse_name,
            (SELECT leaf_sq FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS leaf_sq,
    ...
    public function auctionSellerInvoice()
    {
        ...
        $invoices = $invoice_model->select('auction_seller_invoice.*,seller.name AS seller_name,seller.address,(SELECT delivery_time FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS settings_delivery_time')
    ...
    public function auctionsellerinvoiceDetails($id)
    {
        ...
        $invoice = $invoiceModel->select('auction_seller_invoice.*,auction.date AS auction_date,
        seller.fssai_no AS s_fssai,seller.tea_board_no AS s_tea,seller.gst_no AS s_gst,
        seller.address AS s_address,auction.sale_no,seller.name AS seller_name,invoice.invoice_no AS tax_invoice_no,
        state.name AS s_state_name,city.name AS s_city_name,area.name AS s_area_name,seller.state_id AS s_state_id,
        (SELECT delivery_time FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS settings_delivery_time,
        (SELECT seller_charges FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS settings_seller_charges')
            ->join('auction', 'auction.id = auction_seller_invoice.auction_id', 'left')
            ->join('seller', 'seller.id = auction_seller_invoice.seller_id', 'left')
            ->join('invoice', 'invoice.auction_id = auction_seller_invoice.auction_id', 'left')
        ...
        if ($invoice) {
            ...
            $invoices = $invoiceitems_model->select('auction_seller_invoice_item.bid_price,auction_seller_invoice_item.qty AS pkgs,
            auction_items.id AS auction_id,auction_items.*,inward_items.id AS inward_id,inward_items.*,grade.name AS grade_name,center.name AS center_name,
            garden.name AS garden_name,grade.type AS grade_type,warehouse.name AS warehouse_name,
            (SELECT leaf_sq FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS leaf_sq,
            (SELECT dust_sq FROM settings WHERE status = 1 ORDER BY id DESC LIMIT 1) AS dust_sq
            ')
