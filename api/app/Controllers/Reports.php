...
class Reports extends ResourceController
{
    ...
    public function sellerSoldStockReportSubmit()
    {
        ...
        $invoiceItems = $invoiceItemQuery->groupBy(['invoice.seller_id', 'auction.id'])->orderBy('auction.id')->findAll();
    ...
    public function buyerWiseReportSubmit()
    {
        ...
        $invoiceItems = $invoiceItemQuery->groupBy(['invoice.buyer_id', 'auction.id'])->orderBy('auction.id')->findAll();
    ...
    public function sellerWiseReportSubmit()
    {
        ...
        $invoiceItems = $invoiceItemQuery->groupBy(['invoice.buyer_id', 'auction.id'])->findAll();
    ...
    public function buyerSellerWiseReportSubmit()
    {
        ...
        $query = $invoiceItemModel->select('
            invoice_item.*,invoice.buyer_id,invoice.b_name AS buyer_name,
            auction.sale_no,
            SUM(CASE WHEN auction_items.is_sold = 1 THEN (auction_items.auction_quantity * auction_items.auction_each_net)-auction_items.sample_quantity ELSE 0 END) AS sold_quantity,
            SUM(CASE WHEN auction_items.is_sold = 0 THEN (auction_items.auction_quantity * auction_items.auction_each_net)-auction_items.sample_quantity ELSE 0 END) AS unsold_quantity,
            ...
            (SELECT SUM(CASE WHEN auction_items.is_sold = 1 THEN (auction_items.auction_quantity * auction_items.auction_each_net)-auction_items.sample_quantity ELSE 0 END) 
            where DATE(auction.date) >= "' . UP_SALE_FROM_DATE . '" AND DATE(auction.date) <= "' . UP_SALE_TO_DATE . '") AS upsale_sold_quantity,
            (SELECT SUM(CASE WHEN auction_items.is_sold = 0 THEN (auction_items.auction_quantity * auction_items.auction_each_net)-auction_items.sample_quantity ELSE 0 END) 
            ...
            (SELECT AVG(CASE WHEN auction_items.is_sold = 1 THEN invoice_item.bid_price ELSE NULL END) 
        ...
        if ($garden_id) {
            $query->where('invoice_item.garden_id', $garden_id);
        }
        ...
        $results = $query->groupBy(['invoice_item.garden_id', 'invoice_item.grade_id', 'invoice.buyer_id', 'auction.id'])
            ->findAll();

        // echo $auctionItemModel->getLastQuery();
        // exit;
        $resultsByGardenGrade = ReportsHelper::getTabledata2($results);
    ...
    public function gardenCompareReportSubmit()
    {
        // echo 'hii';exit;
        $auction_id = $this->request->getVar('auction_id');
        $from_date = $this->request->getVar('from_date');
        ...
        ')
            ...
            ->where('DATE(auction.date) <=', $to_date)
            ->whereIn('auction.id', $auction_id)
            ->whereIn('invoice_item.garden_id', $garden_id);

        $results = $query->groupBy(['invoice_item.garden_id', 'invoice_item.grade_id'])
            ->findAll();
    ...
    public function priceRangeWiseReportSubmit()
    {
        ...
        (invoice_item.qty * invoice_item.each_net)-invoice_item.sample_quantity AS qty,
