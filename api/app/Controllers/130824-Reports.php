...
class Reports extends ResourceController
{
    ...
    public function buyerSellerWiseReportSubmit()
    {
        ...
        $query = $invoiceItemModel->select('
            ...
            auction.sale_no,
            SUM(CASE WHEN auction_items.is_sold = 1 THEN auction_items.auction_quantity * auction_items.auction_each_net ELSE 0 END) AS sold_quantity,
            SUM(CASE WHEN auction_items.is_sold = 0 THEN auction_items.auction_quantity * auction_items.auction_each_net ELSE 0 END) AS unsold_quantity,
            ...
            (SELECT SUM(CASE WHEN auction_items.is_sold = 1 THEN auction_items.auction_quantity * auction_items.auction_each_net ELSE 0 END) 
            where DATE(auction.date) >= "' . UP_SALE_FROM_DATE . '" AND DATE(auction.date) <= "' . UP_SALE_TO_DATE . '") AS upsale_sold_quantity,
            (SELECT SUM(CASE WHEN auction_items.is_sold = 0 THEN auction_items.auction_quantity * auction_items.auction_each_net ELSE 0 END) 
