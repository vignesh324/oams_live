...
class Auction extends ResourceController
{
    ...
    public function closeAuction()
    {
        ...
        foreach ($auction_stock as $key => $value) {
            ...
            if ($auction_biddings > 0) {
                ...
                if ($value['reverse_price'] <= $auction_biddings['bid_price']) {
                    ...
                    if ($auction_stock) {
                        $itemsData = $auction_itemmodel->select('auction_items.lot_no,auction_items.auction_each_net,
                        ...
                        grade.type AS grade_type')
                            ...
                            ->join('warehouse', 'warehouse.id = inward.warehouse_id', 'left')
                            ->join('center', 'center.id = inward.center_id', 'left')
                            ->where('auction_items.id', $value['id'])
                            ->first();
        ...
        foreach ($grouped_auction_items as $buyer_id => $buyer_array) {
            ...
            foreach ($buyer_array as $seller_id => $seller_array) {
                ...
                $settingsData1 = $settings_model->orderBy('id', 'DESC')->first();
                ...
                $seller_suffix = sprintf('%04d', $sellerData['seller_suffix']);
                ...
                $invoice_data = [
                    'date' => date('Y-m-d'),
                    'invoice_no' => ucwords($sellerData['seller_prefix']) . '/' . $seller_array[0]['sale_no'] . "/" . $seller_suffix,
                    'buyer_id' => $buyer_id,
                    'seller_id' => $seller_id,
                ...
                $seller_builder = $db->table('seller');
                $seller_builder->where('id', $sellerData['id'])
                    ->update(['seller_suffix' => $sellerData['seller_suffix'] + 1]);
        ...
        foreach ($grouped_auction_items_auctioneer as $buyer_id => $buyer_auctioneer_array) {
            ...
            $settingsData2 = $settings_model->orderBy('id', 'DESC')->first();

            $ab_suffix = sprintf('%04d', $settingsData2['ab_suffix']);

            $invoice_data_auctioneer = [
                'date' => date('Y-m-d'),
                'invoice_no' => ucwords($settingsData['ab_prefix']) . '/'  . $buyer_auctioneer_array[0]['sale_no'] . "/" . $ab_suffix,
                'buyer_id' => $buyer_id,
            ...
            $settings_model->where('id', $settingsData2['id'])->set(['ab_suffix' => $settingsData2['ab_suffix'] + 1])->update();
        ...
        foreach ($grouped_auction_items_seller as $seller_id => $seller_auctioneer_array) {
            ...
            $settingsData3 = $settings_model->orderBy('id', 'DESC')->first();

            $as_suffix = sprintf('%04d', $settingsData3['as_suffix']);

            $invoice_data_seller = [
                'date' => date('Y-m-d'),
                'invoice_no' => ucwords($settingsData['as_prefix']) . '/' . $seller_auctioneer_array[0]['sale_no'] . "/" . $as_suffix,
                'seller_id' => $seller_id,
            ...
            $settings_model->where('id', $settingsData3['id'])->set(['as_suffix' => $settingsData3['as_suffix'] + 1])->update();
    ...
    public function invoiceDetails($id)
    {
        ...
        if ($invoice) {
            ...
            $invoices = $invoiceitems_model->select('invoice_item.*,auction_items.inward_invoice_id')
    ...
    public function closeCurrentAuctionManually()
    {
        ...
        $current_auction = $auctionitemmodel
            ->where('auction_items.auction_id', $auction_id)
            ->findAll();

        foreach ($current_auction as $key => $value) {
            $auction_biddings_count = $auction_biddingmodel->select('bid_price,buyer_id')->where('auction_item_id', $value['id'])->where('auction_biddings.bid_type !=', 3)->countAllResults();

            if($auction_biddings_count == 0){
            ...
            if (isset($auction_biddings)) {
                if ($value['reverse_price'] <= $auction_biddings['bid_price']) {
                    $auctionitemmodel->where('id', $value['id'])->set(['is_sold' => 1])->update();
                }
            }
        }
        ...
        $model->where('id', $id)->set(['status' => 3])->update();
        $auctionitemmodel->where('auction_id', $id)->set(['status' => 2])->update();
