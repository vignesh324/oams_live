...
class JwtLibrary
{
	...
	function generate_jwt($headers, $payload, $secret = 'secret', $expiration = '+4 hours')
	{
		// Set the expiration time to 60 seconds
		$payload['exp'] = strtotime($expiration);
	...
	public function isTokenBlacklisted($token)
    {
        ...
        $result = $db->table('blacklisted_tokens')
            ->where('token', $token)
            ->get()
            ->getRow();
		// print_r($result);exit;
        // If the token exists in the blacklist, return true (token is blacklisted)
        return ($result !== null);
    }
	...
	function is_jwt_valid($jwt, $secret = 'secret')
	{
        ...
        if ($this->isTokenBlacklisted($jwt)) {
            return false;
        }
