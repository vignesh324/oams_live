...
class ReportsHelper
{
    public static function getBaseInvoiceQuery($invoiceItemModel, $from_date, $to_date, $auction_id, $seller_id = null, $garden_id = null, $state_id = null, $city_id = null)
    {
        $invoiceItemModel->select('
            ...
            auction.sale_no,invoice.seller_id,invoice.s_name AS seller_name,
            SUM(CASE WHEN auction.type = 1 THEN invoice_item.qty * invoice_item.each_net ELSE 0 END) AS leaf_total_quantity,
            SUM(CASE WHEN auction.type = 1 THEN invoice_item.qty ELSE 0 END) AS leaf_total_quantity1,
            SUM(CASE WHEN auction.type = 1 THEN invoice_item.qty * invoice_item.bid_price ELSE 0 END) AS leaf_total_bid_price,
            SUM(CASE WHEN auction.type = 2 THEN invoice_item.qty * invoice_item.each_net ELSE 0 END) AS dust_total_quantity,
            SUM(CASE WHEN auction.type = 2 THEN invoice_item.qty ELSE 0 END) AS dust_total_quantity1,
            SUM(CASE WHEN auction.type = 2 THEN invoice_item.qty * invoice_item.bid_price ELSE 0 END) AS dust_total_bid_price,
            COALESCE((SUM(CASE WHEN auction.type = 1 THEN invoice_item.qty * invoice_item.bid_price ELSE 0 END) / NULLIF(SUM(CASE WHEN auction.type = 1 THEN invoice_item.qty ELSE 0 END), 0)), 0) AS leaf_avg_price,
            COALESCE((SUM(CASE WHEN auction.type = 2 THEN invoice_item.qty * invoice_item.bid_price ELSE 0 END) / NULLIF(SUM(CASE WHEN auction.type = 2 THEN invoice_item.qty ELSE 0 END), 0)), 0) AS dust_avg_price
        ')
    ...
    public static function getTabledata2(array $results)
    {
        ...
        foreach ($resultsByGardenGrade as $gardenId => &$gardenData) {
            $totalSoldQuantity = $gardenData['total_sold_quantity1'];
            $totalUpSoldQuantity = $gardenData['total_sold_quantity1'];
            $totalUpSoldQuantity = $gardenData['total_sold_quantity1'];

            foreach ($results as $result) {
                if ($result['garden_id'] == $gardenId) {
                    $percentage = $totalSoldQuantity > 0 ? ($result['sold_quantity1'] / $totalSoldQuantity) * 100 : 0;
                    $upsale_percentage = $totalSoldQuantity > 0 ? ($result['upsale_sold_quantity1'] / $totalSoldQuantity) * 100 : 0;
                    $result['percentage'] = $percentage;
                    $result['upsale_percentage'] = $percentage;
