            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header d-flex justify-content-between ">
                  <h3 class="card-title">Seller wise / Garden wise Buyer purchase qty Reports</h3>
                  <button type="button" id="export" class="btn btn-primary ms-auto  text-end">Export</button>
                  <input type="hidden" value="sellrwise_report" class="excel-file-name">
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <div class="row">
    ...
    <script>
      $(document).ready(function() {
        ...
        function handleSaleNoChange() {
          ...
          $.ajax({
            url: "<?= @basePath ?>USER/SalenoWiseSeller",
            type: "POST",
            data: {
              auction_id: auction_id
            },
            dataType: 'json',
            success: populateSellers,
            error: handleError('#seller_id', 'No data found')
          });
        }

        function populateSellers(response) {
          var $sellerId = $("#seller_id");
          var $gardenId = $("#garden_id");

          $sellerId.empty();
          $gardenId.empty().append('<option value="">Select Garden</option>');

          if (response.seller.length > 0) {
            $sellerId.append('<option value="">Select Seller</option>');
            response.seller.forEach(function(value) {
              var option = `<option value="${value.id}">${value.name}</option>`;
              $sellerId.append(option);
            });
          } else {
            $sellerId.append('<option value="">No data found</option>');
          }
        }
        ...
        function handleSellerChange() {
          ...
          $.ajax({
            url: "<?= @basePath ?>USER/SellerWiseGarden",
            type: "POST",
            data: {
              seller_id: seller_id
            },
        ...
        function populateGardens(response) {
          ...
          if (response.data.sellerGarden.length > 0) {
            $gardenId.append('<option value="">Select Garden</option>');
            response.data.sellerGarden.forEach(function(value) {
              var option = `<option value="${value.id}">${value.name}</option>`;
        ...
        function handleSearchFilterClick(event) {
          ...
          $.ajax({
            url: '<?= @basePath ?>USER/SellerWiseReportSubmit',
            type: 'post',
            dataType: 'html',
            data: {
              from_date: from_date,
