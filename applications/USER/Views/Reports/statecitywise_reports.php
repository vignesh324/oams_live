                    ...
                    <div class="col col-2">
                      <div class="form-group">
                        ...
                        <select class="form-control" name="state_id" id="state_id">
                          ...
                          <?php foreach ($state_list['state'] as $key => $val) : ?>
    ...
    <script>
      $(document).ready(function() {
        ...
        function handleSaleNoChange() {
          ...
          $.ajax({
            url: "<?= @basePath ?>USER/SalenoWiseState",
            type: "POST",
            data: {
              auction_id: auction_id
            },
            dataType: 'json',
            success: populateState,
            error: handleError('#state_id', 'No data found')
          });
        }

        function populateState(response) {
          var $gardenId = $("#state_id");
          $gardenId.empty();

          if (response.state.length > 0) {
            $gardenId.append('<option value="">Select State</option>');
            response.state.forEach(function(value) {
        ...
        function handleSearchFilterClick(event) {
          ...
          if (!state_id) {
            displayError('state_id', 'Please select state');
            hasError = true;
          }
          ...
          $.ajax({
            url: '<?= @basePath ?>USER/StateCityReportSubmit',
            type: 'post',
            dataType: 'html',
            data: {
              from_date: from_date,
