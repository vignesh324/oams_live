...
<body class="hold-transition sidebar-mini">
  <div class="wrapper">
    ...
    $warehouse_permission = array_values($filtered_permissions);
              ...
              <div class="card">
                ...
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      <?php foreach ($response_data['warehouse'] as $key => $value) : ?>
                        <tr>
                          <td><?php echo $key + 1; ?></td>
                          <td><?php echo $value['name']; ?></td>
                          <td><?php echo WAREHOUSE . $value['id']; ?></td>
    ...
    <script>
      function editWarehouse(id) {
        ...
        $.ajax({
          type: "post",
          url: "<?= @basePath ?>USER/Warehouse/Show",
          data: {
            id: id
          },
          dataType: 'html',
      ...
      function addWarehouse() {
        ...
        $.ajax({
          type: "post",
          url: "<?= @basePath ?>USER/Warehouse/Add",
          dataType: 'html',
          success: function(response) {
            $(".loading").hide();
            $(".modal-content").html(response);
      ...
      function deleteWarehouse(id) {
        ...
        swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            $.ajax({
              type: "post",
              url: "<?= @basePath ?>USER/Warehouse/Delete",
              data: {
                id: id
              },
              dataType: 'html',
      ...
      $(document).on("click", "#add-warehouse", function(event) {
        ...
        $("#add-warehouse").attr("disabled", true);

        var url = '<?= @basePath ?>USER/Warehouse/Create';
        var formmethod = 'post';
        var formdata = $('form#warehouse-form').serialize();
        ...
        $.ajax({
          ...
          complete: function() {
            ...
            $("#add-warehouse").attr("disabled", false);
          }
        });
      });


      $(document).on("click", "#edit-warehouse", function(event) {
        event.preventDefault();
        $("#edit-warehouse").attr("disabled", true);

        var url = '<?= @basePath ?>USER/Warehouse/Update';
        var formmethod = 'post';
        var formdata = $('form#warehouse-form-edit').serialize();
        console.log(formdata);
        $.ajax({
          url: url,
          type: formmethod,
          ...
          complete: function() {
            // Re-enable the submit button after the request is complete
            $("#edit-warehouse").attr("disabled", false);
          }
        });
      });
    </script>
